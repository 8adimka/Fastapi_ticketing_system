...
class TicketService(
    CRUDBase[
        ticket_model.Ticket, ticket_schema.TicketCreate, ticket_schema.TicketUpdate
    ]
):
    async def get(self, db: Session, item_id: UUID) -> Optional[ticket_model.Ticket]:
        ...
        """
        ticket = await self.cache.get_from_cache(
            obj_id=str(item_id), obj_model=ticket_schema.Comment
        )
        if not ticket:
    ...
    async def create(
        self, db: Session, *, obj_in: ticket_schema.TicketCreate
    ) -> ticket_model.Ticket:
        """Create a ticket
    ...
    async def update(
        ...
        db: Session,
        *,
        db_obj: ticket_model.Ticket,
        obj_in: Union[ticket_schema.TicketUpdate, Dict[str, Any]],
    ) -> ticket_model.Ticket:
        """[summary]
        ...
        if db_obj.status in [
            ticket_model.TicketStatus.ANSWERED,
            ticket_model.TicketStatus.WAIT_ANSWER,
        ]:
